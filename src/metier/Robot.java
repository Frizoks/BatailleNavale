...
public class Robot extends Adversaire
{
	...
	public char jouerCoupDifficile()
	{
		...
		if(this.coupPossible.size() > 0)
		{
			...
			if(dernierCoup.size() >= 2) {delta = dernierCoup.get(0)[0] == dernierCoup.get(1)[0] ? 'X' : 'Y';}
			...
			{
				for(int[] tab : Robot.voisinTir) {coupPossible.remove(tab);}
				Robot.voisinTir.clear();
				dernierCoup.clear();
				delta = 'N';
			...
			else
			{
				...
				{
					if(coupUtile(tab[0], tab[1]) == coupOpti) {encore = true;}
				}
				if(!encore) {coupOpti--;}

				for(int[] coup : coupPossible)
				{
					if(coupUtile(coup[0], coup[1]) == coupOpti) {coupTopTier.add(coup);}
				}
	...
	{
		if(coup[0] >= Plateau.LARG && coup[0] < 0 || coup[1] >= Plateau.LARG && coup[1] < 0) {return true;}

		for(int[] tab : coupPossible)
		{
			if(coup[0] == tab[0] && coup[1] == tab[1]) {return true;}
		}
		return false;
	}

	/**
	 * Ajoute aléatoirement les navires du robot sur le plateau de jeu.
	 */

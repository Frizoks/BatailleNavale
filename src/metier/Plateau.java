...
{
	public final static int LARG = 8;
	...
	{
		if(x > 0 && plateau[x - 1][y].getNavire() && !(x < (Plateau.LARG - 1) && plateau[x + 1][y].getNavire())) {return 'G';}
		if(x < (Plateau.LARG - 1) && plateau[x + 1][y].getNavire() && (x > 0 && plateau[x - 1][y].getNavire())) {return 'D';}
		if(y > 0 && plateau[x][y - 1].getNavire() && (y < (Plateau.LARG - 1) && plateau[x][y + 1].getNavire())) {return 'H';}
		if(y < (Plateau.LARG - 1) && plateau[x][y + 1].getNavire() && (y > 0 && plateau[x][y - 1].getNavire())) {return 'B';}
		return 'R';
	}

	public char joueCoup(int x, int y)
	{
	...
	public boolean navireEntier(int x, int y)
	{
		...
		if(x > 0)          {if(plateau[x - 1][y].getNavire()) {deltaX = true;};}
		if(x < (Plateau.LARG - 1)) {if(plateau[x + 1][y].getNavire()) {deltaX = true;};}
		if(y > 0)          {if(plateau[x][y - 1].getNavire()) {deltaY = true;};}
		if(y < (Plateau.LARG - 1)) {if(plateau[x][y + 1].getNavire()) {deltaY = true;};}
		...
		while(!estEntier)
		{
			...
			if((!svt.getNavire()) || (x >= Plateau.LARG || y >= Plateau.LARG)) {estEntier = true;}
			else 
			{

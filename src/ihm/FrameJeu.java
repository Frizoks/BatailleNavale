	...
	public void partieFini(char c)
	{
		...
		if(c == 'J') 
		{
			...
			try {
				...
				File file = new File("src\\ihm\\image\\win.wav");
		...
		else
		{
			...
			try {
				// Open an audio input stream.
				File file = new File("src\\ihm\\image\\loose.wav");
				AudioInputStream audioIn = AudioSystem.getAudioInputStream(file);
				// Get a sound clip resource.
				Clip clip = AudioSystem.getClip();
				// Open audio clip and load samples from the audio input stream.
		...
		if ( this.ctrl.estMulti() ) { JOptionPane.showMessageDialog(this, c == 'J' ? "Vous avez gagné" : "L'adversaire a gagné", "Fin de partie", JOptionPane.INFORMATION_MESSAGE); }
		else {
			//Création d'une "Pop-up" pour demander si le joueur veux rejouer ou quitter
			Object[] choix= { "Acceuil","Quitter" };
			int rep = JOptionPane.showOptionDialog ( this, c == 'J' ? "Vous avez gagné" : "L'adversaire a gagné", "Fin de partie", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, choix, choix[0] );

...
public class PanelJeu extends JPanel implements MouseListener
{
	...
	private int nbNvairesToucheJoueur;
	private int nbCoupAdv;
	private int nbNvairesToucheAdv;
	...
	public PanelJeu ( FrameJeu frmJeu )
	{
		...
		this.nbCoupJoueur = this.nbNvairesToucheJoueur = 0;
		this.nbCoupAdv = this.nbNvairesToucheAdv = 0;
	...
	public void grilleAdvActive() { this.grilleAdvEstActive = true; }
	...
	public void mouseClicked(MouseEvent e)
	{
		...
		if ( zoneAdv.contains ( e.getPoint ( ) ) && this.grilleAdvEstActive)
		{
			...
			if(cj != 'R')
			{
				switch(cj)
				{
					case 'E' : ((PanelMessage) this.frmJeu.pnlBas).ajouterLogsJ("Vous avez tiré dans l'eau en " + (char)('A' + (e.getX()-400)/25) +((e.getY()-75)/25+1));break;
					case 'T' : ((PanelMessage) this.frmJeu.pnlBas).ajouterLogsJ("Vous avez touché un navire en " + (char)('A' + (e.getX()-400)/25) +((e.getY()-75)/25+1));break;
					case 'C' : ((PanelMessage) this.frmJeu.pnlBas).ajouterLogsJ("Vous avez coulé un navire en " + (char)('A' + (e.getX()-400)/25) +((e.getY()-75)/25+1));break;
				}
				this.nbCoupJoueur++;
				if(cj != 'E') {this.nbNvairesToucheJoueur++;}
				
				this.grilleAdvEstActive = false;

				((PanelMessage) this.frmJeu.pnlBas).ajouterScoreJ("Vous avez coulé " + this.frmJeu.getCtrl().getNbNaviresCouleJoueur() + " navires", 
																  "Avec une précision de " + Math.round(this.nbNvairesToucheJoueur/(float)this.nbCoupJoueur*100) + " %");

				if ( this.frmJeu.getCtrl().quelqunGagne() == 'J' )
					this.frmJeu.partieFini('J');

				char ca = this.frmJeu.getCtrl().advJoue();
				switch(ca)
				{
				...
				this.nbCoupAdv++;
				if(ca != 'E') {this.nbNvairesToucheAdv++;}

				((PanelMessage) this.frmJeu.pnlBas).ajouterScoreA("Votre adversaire a coulé " + this.frmJeu.getCtrl().getNbNaviresCouleAdv() + " navires", 
																  "Avec une précision de " + Math.round(this.nbNvairesToucheAdv/(float)this.nbCoupAdv*100) + " %");

				if ( this.frmJeu.getCtrl().quelqunGagne() == 'A' ) {this.frmJeu.partieFini('A');}
		...
		if ( zoneJoueur.contains ( e.getPoint ( ) ))
		{
			try {

...
public class Sound
{
	...
	public Sound()
	{
		if(!init)
		{
			...
			if(!Sound.sound) {System.out.println("Vous ne pouvez actuellement pas jouez de son, pour le faire veuillez placer le dossier \"dataBN\" avec le fichier Jar, plus d'information dans le README.md, le tout disponible sur Github(Frizoks)");}
	...
	public static void soundCharge(String fichier)
	{
		try{
			...
			try{
				...
				File file = os == "linux" ? new File("src/ihm/image/" + fichier + ".wav") :new File("src\\ihm\\image\\" + fichier + ".wav");
				audioIn = AudioSystem.getAudioInputStream(file);
			}catch(Exception e)
			{
				try{
					// Open an audio input stream.
					File file = os == "linux" ? new File("dataBN/" + fichier + ".wav") :new File("dataBN\\" + fichier + ".wav");
					audioIn = AudioSystem.getAudioInputStream(file);
				}catch(Exception e2) {Sound.sound = false;}
			}
